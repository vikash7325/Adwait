package android.adwait.com.utils;

public class ADConstants {

    public static final int MENU_HOME = 0;
    public static final int MENU_MY_MENTEE = 1;
    public static final int MENU_BE_THE_CHANGE = 2;
    public static final int MENU_WISH_CORNER = 3;
    public static final int MENU_HX_CLUB = 4;
    public static final int MENU_MY_CONTRIBUTION = 5;
    public static final int MENU_OUR_PARTNERS = 6;
    public static final int MENU_REFER_HAPPINESS = 7;
    public static final int MENU_OUR_CAUSE = 8;
    public static final int MENU_CONTACT_US = 9;
    public static final int MENU_LOGOUT = 10;
    public static final int MENU_DONATION = 11;
    public static final int MENU_PROFILE = 12;
    public static final String ADMIN_REFERRAL_CODE="Adm!n@2019";
    public static final String SUPER_ADMIN_NAME="adwaitkunwar2019@gmail.com";
    public static final String SUPER_ADMIN_PASS="Adm!n";

    //Animation
    public static final int ANIMATION_COUNT = 500;
    public static final int ANIMATION_SIZE = 12;
    public static final long ANIMATION_TIME_TO_LIVE = 2000L;
    public static final long ANIMATION_EMITTING_TIME = 1500L;
}
